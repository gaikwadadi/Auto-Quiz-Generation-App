    ...
    if "structured_questions" in st.session_state:
        ...
        for idx, q in enumerate(st.session_state["structured_questions"], 1):
            ...
            if st.session_state["quiz_type"] == "multiple-choice":
                ...
                user_answer = st.radio("Choose one:", options, key=f"q{idx}", index=0)
                user_answers[str(idx)] = None if user_answer == "--Select--" else user_answer

            elif st.session_state["quiz_type"] == "true-false":
                options = ["--Select--", "True", "False"]
                user_answer = st.radio("Choose:", options, key=f"q{idx}", index=0)
                user_answers[str(idx)] = None if user_answer == "--Select--" else user_answer
